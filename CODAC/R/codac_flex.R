#' Execute the CODAC_Flex Analysis Pipeline
#'
...
#' @param targets_to_plot Character vector. Specific genes to plot (e.g., c("Per2", "Arntl")).
...
#' @param p_value_option Multiple testing correction: 'FDR' or 'RAW' (default: 'FDR').
...
#'   For entrained (ZT), single-cycle data 'fixed' is recommended; 'variable' is
#'   intended for free-running / multi-cycle data.
...
#' @param plot_flag Character. "Y" to generate individual plots for each target (default: "N").
#' @param plot_all Character. "Y" to plot all genes, "N" to plot only those in targets_to_plot (default: "Y").
#' @param time_label X-axis label in plots: 'ZT', 'CT', or 'Clock' (default: 'ZT').
#'
#' @return A dataframe with the rhythmicity results (incl. Winning_Model, AICc and Flex_Parameter).
#' @export
codac_flex <- function(data,
                          timepoints,
                          n_observations = 1,
  ...
  script_path <- system.file("python", "CODAC_Flex_Py.py", package = "CODAC")

  if (script_path == "") {
    stop("[ERROR] Python script not found. Ensure 'CODAC_Flex_Py.py' is in the 'inst/python' folder.")
  }

  message("\n[INFO] Starting the CODAC_Flex Pipeline via Python engine...")
  ...
  message("[SUCCESS] Flex Pipeline finished successfully!")
