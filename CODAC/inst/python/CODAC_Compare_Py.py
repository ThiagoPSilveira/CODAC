...
def read_input_file(input_file):
    ...
    if len(groups) < 2:
        raise ValueError('For the Compare version, you must define at least 2 groups in Input.txt.')
...
# -------------------------------------------------------------------
# Compute Differential Statistics (The "Compare" Heart)
# -------------------------------------------------------------------
...
def generate_heatmap_compare(results, df_long, n_observations,counter_threshold, base_dir, time_label='Time (ZT Hours)'):
    ...
    for grp in groups:
        ...
        show_labels = (cell_px >= 6) and (n_genes <= max_genes_heatmap_labels)
...
def generate_heatmap_by_category(df_comparisons, results, df_long, n_observations,
    ...
    def draw_one(matrix_z, time_labels, gene_order, title, out_path, fig_w, fig_h, label_w):
        ...
        max_genes_heatmap_labels = 50
        show_labels = n_genes <= max_genes_heatmap_labels
...
def main():
    ...
    if is_r_injected:
        ...
        input_file = os.path.join(base_dir, 'compare_input.txt')
        name_file = None
